 ...
 * 循迹控制（Track）
 * 
 * Track模块采用状态机控制方式（非PID），由 Track_Update() 根据传感器状态输出
 * Track_Error，直接映射到 TurnPID.Target。
 * 
 * 控制层级如下：
 * 
 *   Track_Error --[×0.5]-> TurnPID.Target --[DifPWM]-> 差分PWM(左右转向)
 *   Track_Speed --[Target]-> SpeedPID --[Target]-> AnglePID -> AvePWM(前后行进)
 *   AnglePID --[AvePWM + DifPWM]-> 左右电机PWM -> 平衡车姿态
 * 
 * 状态说明（详见Track.c的Track_Update函数注释）：
 *   S2/S3在线 → Error=0 → 直行不调（平衡车精度差，小偏差调整反而晃动更大）
 *   S1/S4压线 → Error=±30 → 强制转向找回
 *   过渡滑行 → Error=0 → 继续直行（线圈仍在传感器覆盖区）
 *   脱轨搜索 → Error=±20~±40 → 周期摆动搜索黑线
 **************************************************************************************/
	...
	while (1)
	{
		...
		if (KeyNum == 2)			//如果K2按下（PB0）：切换循迹模式
		{
			...
			if (TrackModeFlag)					//如果开启循迹模式
			{
				...
				Track_ResetState();				//复位循迹内部状态（首次运行标志、脱轨计数等）
		...
		{
			/* OLED中间显示循迹控制状态（取代速度环显示） */
			OLED_Printf(38, 0, OLED_6X8, "Track");					//OLED中间显示"Track"
			OLED_Printf(38, 8, OLED_6X8, "Err:%+04.0f", Track_Error);//循迹误差/转向指令
			OLED_Printf(38, 16, OLED_6X8, "Spd:%05.2f", Track_Speed);//循迹速度
			/* 显示传感器状态：|S1|S2|S3|S4|，*=黑线，-=白面 */
			OLED_Printf(38, 24, OLED_6X8, "|%c|%c|%c|%c|",
				(Track_SensorValues & 0x08) ? '*' : '-',
				(Track_SensorValues & 0x04) ? '*' : '-',
				(Track_SensorValues & 0x02) ? '*' : '-',
				(Track_SensorValues & 0x01) ? '*' : '-');
			OLED_Printf(38, 32, OLED_6X8, "TurnT:%+04.0f", TurnPID.Target);//转向目标
			OLED_Printf(38, 40, OLED_6X8, "Dif:%+04.0f", DifPWM);	//差分PWM
			OLED_Printf(38, 48, OLED_6X8, "Mode:TRACK");			//模式指示
		}
	...
	if (TIM_GetITStatus(TIM1, TIM_IT_Update) == SET)
	{
		...
		if (Count1 >= 50)		//如果计次50次，则if成立，即if每隔50ms进一次
		{
			...
			if (RunFlag)					//RunFlag非0时，启动PID程序
			{
				/* ========== 循迹模式下的控制 ========== */
				/*
				 * Track_Update是状态机，输出Track_Error直接用于控制：
				 *   Track_Error=0   → S2/S3在线或滑行过渡 → 直行不调
				 *   Track_Error=±30 → S1/S4外圈压线 → 转向找回
				 *   Track_Error=±20~±40 → 脱轨搜索模式
				 * 
				 * 方向映射（Track_Error直接控制转向目标）：
				 *   Track_Error<0（负）→ 线偏左 → 左转 → TurnPID.Target<0
				 *   Track_Error>0（正）→ 线偏右 → 右转 → TurnPID.Target>0
				 *   0.5为缩放系数：±30 → ±15，经TurnPID输出约±50~60的DifPWM
				 * 
				 * 循迹速度控制：脱轨时减速便于找回，在线时恢复正常速度。
				 */
				if (Track_Enabled)
				{
					float TrackEffectiveSpeed;
					if (Track_IsLost())
						TrackEffectiveSpeed = TRACK_SEARCH_SPEED;
					else
						TrackEffectiveSpeed = Track_GetSpeed();
					
					SpeedPID.Target = TrackEffectiveSpeed;
					TurnPID.Target = Track_Error * 0.5f;	//直接映射到转向目标
				}
				else						//遥控模式（原有逻辑）
				{
